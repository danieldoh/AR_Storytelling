...
def run_follow(model, condition_list):
    for cond in condition_list:
        ...
        while not(os.path.exists(posepath)):
            print(f"{posepath}: No directory \n")
            pose_folder = input("Please provide pose folder name: ")
            pose_name = input("Please provide pose video name(e.g., [action]_0[0/1/2].mp4): ")
            posepath = os.path.join("/workspace/animation_from_TP/pose_video", pose_folder, pose_name)
        ...
        subprocess.run([f"TORCH_DISTRIBUTED_DEBUG=DETAIL accelerate launch txt2video.py --config=\"/workspace/animation_from_TP/condition.yaml\" --skeleton_path=\"/workspace/animation_from_TP/pose_video/{pose_folder}/{pose_name}\" --file_name=\"{save_name}\""], shell=True, cwd=working_directory)
